...
class FullstaceJsonFiller(object):
    ...
    def __init__(self, first_iteration, last_iteration, grid_last_iteration,
                       inpath, outpath,
        ...
        self.grid_last_iteration = grid_last_iteration
        self.n_iterations = self.last_iteration - self.first_iteration + 1
    ...
    def fill_descriptor(self):
        ...
        operations = []
        for iteration_index in range(self.first_iteration, self.last_iteration+1):
            # Filling climatology
    ...
    def fill_batch_descriptor(self):
        ...
        for iteration_index in range(self.first_iteration, self.last_iteration+1):
            ...
            if (iteration_index==self.last_iteration) and self.grid_last_iteration:
            ...
            if (iteration_index==self.last_iteration) and self.grid_last_iteration:
                grid_operation = self.fill_batch_output_gridding(iteration_index)
    ...
    def fill_output_gridding(self, iteration_index):
        ...
        for storage, solution_datasetname, uncertainty_datasetname, sample_datasetname, sample_prior_datasetname in zip(FullstaceJsonFiller.STORAGE_LIST, 
            ...
            self.output_gridding_dict['runmodule'][storage]['statefilename_read']['datasetname'] = solution_datasetname+'_'+str(iteration_index)
            #self.output_gridding_dict['runmodule'][storage]['marginal_std_filename_read']['datasetname'] = uncertainty_datasetname
            self.output_gridding_dict['runmodule'][storage]['statefilename_read']['datasetname'] = solution_datasetname+'_'+str(iteration_index)
            self.output_gridding_dict['runmodule'][storage]['sample_filename_read']['datasetname'] = solution_datasetname+'_sample_'+str(iteration_index)#sample_datasetname
            self.output_gridding_dict['runmodule'][storage]['prior_sample_filename_read']['datasetname'] = solution_datasetname+'_prior_sample_'+str(iteration_index)#sample_prior_datasetname
    ...
    def fill_batch_output_gridding(self, iteration_index):
        """Fill final output gridding operation"""
       
        new_dict = copy.deepcopy(self.batch_operation_dict)
       
        self.fill_primary_keys(new_dict, self.output_grid_command+'_'+str(iteration_index), self.output_grid_name+'_'+str(iteration_index), 'nc', 'measurement')
        ...
        new_dict['runmodule']['outputfiles'] = { 'python_class' : 'eustace.analysis.advanced_standard.examples.placeholder.AnnualBatchDaysOutput', 'datasetname' : self.output_grid_name+'_'+str(iteration_index)  }
        new_dict['runmodule']['climatologyfiles'] = { 'python_class' : 'eustace.analysis.advanced_standard.examples.placeholder.AnnualBatchDaysOutput', 'datasetname' : self.output_climatology_name+'_'+str(iteration_index)  }
        new_dict['runmodule']['largescalefiles'] = { 'python_class' : 'eustace.analysis.advanced_standard.examples.placeholder.AnnualBatchDaysOutput', 'datasetname' : self.output_large_scale_name+'_'+str(iteration_index)  }
        new_dict['runmodule']['localfiles'] = { 'python_class' : 'eustace.analysis.advanced_standard.examples.placeholder.AnnualBatchDaysOutput', 'datasetname' : self.output_local_name+'_'+str(iteration_index)  }
        ...
        for storage, solution_datasetname, uncertainty_datasetname, sample_datasetname, sample_prior_datasetname in zip(FullstaceJsonFiller.STORAGE_LIST, 
            ...
            if solution_datasetname != self.solution_local_scale_name:
                ...
                new_dict['runmodule'][storage]['sample_filename_read'] = {'python_class' : "eumopps.catalogue.placeholder.InputFile",
                                                                          'datasetname': solution_datasetname+'_sample_'+str(iteration_index)}
                new_dict['runmodule'][storage]['prior_sample_filename_read'] = {'python_class': 'eumopps.catalogue.placeholder.InputFile',
                                                                                'datasetname':   solution_datasetname+'_prior_sample_'+str(iteration_index)}
            else:
        ...
        climatology_dataset['name'] = self.output_climatology_name+'_'+str(iteration_index)
        subsets = climatology_dataset['subsets'][0]
        subsets['layout']['patterns'] = FullstaceJsonFiller.return_output_patterns(self.output_climatology_name+'_'+str(iteration_index), 'nc', 'measurement')
        new_dict['newdatasets'].append( climatology_dataset )
        
        large_scale_dataset = copy.deepcopy( new_dict['newdatasets'][0] )
        large_scale_dataset['name'] = self.output_large_scale_name+'_'+str(iteration_index)
        subsets = large_scale_dataset['subsets'][0]
        subsets['layout']['patterns'] = FullstaceJsonFiller.return_output_patterns(self.output_large_scale_name+'_'+str(iteration_index), 'nc', 'measurement')
        new_dict['newdatasets'].append( large_scale_dataset )
        
        local_dataset = copy.deepcopy( new_dict['newdatasets'][0] )
        local_dataset['name'] = self.output_local_name+'_'+str(iteration_index)
        subsets = local_dataset['subsets'][0]
        subsets['layout']['patterns'] = FullstaceJsonFiller.return_output_patterns(self.output_local_name+'_'+str(iteration_index), 'nc', 'measurement')
