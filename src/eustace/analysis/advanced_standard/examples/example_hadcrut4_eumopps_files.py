...
class AnalysisSystem_HadCRUT4(AnalysisSystem):

    def __init__(self, storage_climatology, storage_large_scale, storage_local):
        ...
        super(AnalysisSystem_HadCRUT4, self).__init__(

            components = [ 
                SpaceTimeComponent(HadCRUT4_InMemory.definition_climatology, storage_climatology),
                SpaceTimeComponent(HadCRUT4_InMemory.definition_large_scale, storage_large_scale),
                SpatialComponent(HadCRUT4_InMemory.definition_local, storage_local) ],

            observable = ObservationSource.TMEAN)

def process_month(storage_climatology, storage_large_scale, storage_local, inputfilelist, time_index, component_index, processdate):
    
    inputloaders = [ AnalysisSystemInputLoaderHadCRUT4(inputfilelist) ]
    AnalysisSystem_HadCRUT4(storage_climatology, storage_large_scale, storage_local).update_component_time(inputloaders, component_index, time_index)

def solve(storage_climatology, storage_large_scale, storage_local, component_index):

    AnalysisSystem_HadCRUT4(storage_climatology, storage_large_scale, storage_local).update_component_solution(component_index)

def output_month(storage_climatology, storage_large_scale, storage_local, outputfile, time_index, processdate):
    ...
    result_expected_value = AnalysisSystem_HadCRUT4(storage_climatology, storage_large_scale, storage_local).evaluate_expected_value(outputstructure)
...
def main():
    ...
    catalogue = eumopps.catalogue.catalogue.Catalogue(
        ...
        datasets = [
            ...
            # Produce climatology measurements per-month
            eumopps.catalogue.dataset.CatalogueDataSet(
                name='measurement_climatology',
                path=outputpath,
                subsets = [
                    eumopps.catalogue.dataset.CatalogueDataSubset(layout=eumopps.catalogue.storage.DataStorageFiles(patterns=[ 'example_hadcrut4_measurement_climatology_%Y%m.pickle' ])),
                    ]),

            # Add to get climatology state vector
            eumopps.catalogue.dataset.CatalogueDataSet(
                name='state_climatology',
                path=outputpath,
                subsets = [
                    eumopps.catalogue.dataset.CatalogueDataSubset(layout=eumopps.catalogue.storage.DataStorageFiles(patterns=[ 'example_hadcrut4_state_climatology.pickle' ])),
                    ]),

            # Then large scale measurements per-month
            eumopps.catalogue.dataset.CatalogueDataSet(
                name='measurement_large_scale',
                path=outputpath,
                subsets = [
                    eumopps.catalogue.dataset.CatalogueDataSubset(layout=eumopps.catalogue.storage.DataStorageFiles(patterns=[ 'example_hadcrut4_measurement_large_scale_%Y%m.pickle' ])),
                    ]),
            
            # Initialised value of large-scale state vector
            eumopps.catalogue.dataset.CatalogueDataSet(
                name='initial_state_large_scale',
                path=outputpath,
                subsets = [
                    eumopps.catalogue.dataset.CatalogueDataSubset(layout=eumopps.catalogue.storage.DataStorageFiles(patterns=[ 'example_hadcrut4_initstate_large_scale.pickle' ])),
                    ]),

            # Large-scale state vector
            eumopps.catalogue.dataset.CatalogueDataSet(
                name='state_large_scale',
                path=outputpath,
                subsets = [
                    eumopps.catalogue.dataset.CatalogueDataSubset(layout=eumopps.catalogue.storage.DataStorageFiles(patterns=[ 'example_hadcrut4_state_large_scale.pickle' ])),
                    ]),
            
            # Local measurements (just compute state directly)
            eumopps.catalogue.dataset.CatalogueDataSet(
                name='state_local',
                path=outputpath,
                subsets = [
                    eumopps.catalogue.dataset.CatalogueDataSubset(layout=eumopps.catalogue.storage.DataStorageFiles(patterns=[ 'example_hadcrut4_state_local_%Y%m.pickle' ])),
                    ]),
            
            # The output data set to make
            eumopps.catalogue.dataset.CatalogueDataSet(
                name='infilled',
                path=outputpath,
                subsets = [
        ...
        operations = [
            ...
            eumopps.catalogue.operation.Operation(
                ...
                runmodule = {
                    'python_function': 'eustace.analysis.advanced_standard.examples.example_hadcrut4_eumopps_files.process_month',
                    'storage_climatology': {
                        'python_class': 'eustace.analysis.advanced_standard.components.storage_files.SpaceTimeComponentSolutionStorage_Files',
                        'measurement_time_index_write':  eumopps.catalogue.placeholder.StepIndex(),
                        'measurementfilename_write' : eumopps.catalogue.placeholder.OutputFile('measurement_climatology')
                        },
                    'storage_large_scale': {
                        'python_class': 'eustace.analysis.advanced_standard.components.storage_files.SpaceTimeComponentSolutionStorage_Files',
                        },
            ...
            eumopps.catalogue.operation.Operation(
                ...
                runmodule = {
                    'python_function': 'eustace.analysis.advanced_standard.examples.example_hadcrut4_eumopps_files.solve',
                    'storage_climatology': {
                        'python_class': 'eustace.analysis.advanced_standard.components.storage_files.SpaceTimeComponentSolutionStorage_Files',
                        'measurementfilelist_read' : eumopps.catalogue.placeholder.InputFileList('measurement_climatology'),
                        'statefilename_write' : eumopps.catalogue.placeholder.OutputFile('state_climatology')
                        },
                    'storage_large_scale': {
                        'python_class': 'eustace.analysis.advanced_standard.components.storage_files.SpaceTimeComponentSolutionStorage_Files',
                        },
            ...
            eumopps.catalogue.operation.Operation(
                ...
                runmodule = {
                    ...
                    'storage_climatology': {
                        'python_class': 'eustace.analysis.advanced_standard.components.storage_files.SpaceTimeComponentSolutionStorage_Files',
                        'statefilename_read' : eumopps.catalogue.placeholder.InputFile('state_climatology')
                        },
                    'storage_large_scale': {
                        'python_class': 'eustace.analysis.advanced_standard.components.storage_files.SpaceTimeComponentSolutionStorage_Files',
                        'measurement_time_index_write':  eumopps.catalogue.placeholder.StepIndex(),
                        'measurementfilename_write' : eumopps.catalogue.placeholder.OutputFile('measurement_large_scale'),
                        'statefilename_write' : eumopps.catalogue.placeholder.OutputFile('initial_state_large_scale')
                        },
                    'storage_local': {
                        'python_class': 'eustace.analysis.advanced_standard.components.storage_files.SpatialComponentSolutionStorage_Files',
                        },
            ...
            eumopps.catalogue.operation.Operation(
                ...
                runmodule = {
                    ...
                    'storage_large_scale': {
                        'python_class': 'eustace.analysis.advanced_standard.components.storage_files.SpaceTimeComponentSolutionStorage_Files',
                        'measurementfilelist_read' : eumopps.catalogue.placeholder.InputFileList('measurement_large_scale'),
                        'statefilename_read' : eumopps.catalogue.placeholder.InputFile('initial_state_large_scale'),
                        'statefilename_write' : eumopps.catalogue.placeholder.OutputFile('state_large_scale')
                        },
                    'storage_local': {
                        'python_class': 'eustace.analysis.advanced_standard.components.storage_files.SpatialComponentSolutionStorage_Files',
                        },
            ...
            eumopps.catalogue.operation.Operation(
                ...
                runmodule = {
                    ...
                    'storage_large_scale': {
                        'python_class': 'eustace.analysis.advanced_standard.components.storage_files.SpaceTimeComponentSolutionStorage_Files',
                        'statefilename_read' : eumopps.catalogue.placeholder.InputFile('state_large_scale')
                        },
                    'storage_local': {
                        'python_class': 'eustace.analysis.advanced_standard.components.storage_files.SpatialComponentSolutionStorage_Files',
                        'time_index': eumopps.catalogue.placeholder.StepIndex(),
                        'statefilename_write' : eumopps.catalogue.placeholder.OutputFile('state_local')
                        },
                    'inputfilelist':  [ 
                        eumopps.catalogue.placeholder.InputFile('hadcrut4', 0),
                        eumopps.catalogue.placeholder.InputFile('hadcrut4', 1),
            ...
            eumopps.catalogue.operation.Operation(
                name = 'output_grid',
                runmodule = {
                    'python_function': 'eustace.analysis.advanced_standard.examples.example_hadcrut4_eumopps_files.output_month',
                    'storage_climatology': {
                        'python_class': 'eustace.analysis.advanced_standard.components.storage_files.SpaceTimeComponentSolutionStorage_Files',
                        'statefilename_read' : eumopps.catalogue.placeholder.InputFile('state_climatology')
                        },
                    ...
                    'storage_local': {
                        'python_class': 'eustace.analysis.advanced_standard.components.storage_files.SpatialComponentSolutionStorage_Files',
                        'time_index': eumopps.catalogue.placeholder.StepIndex(),
                        'statefilename_read' : eumopps.catalogue.placeholder.InputFile('state_local')
                        },
    ...
    for operationgroup in [
        ...
        print 'Processing: {0} ({1} operations)'.format(operationgroup, operation_count)
