#!/bin/bash
#
# submit_masking_window.sh
#
# Use flags from submit_threshold_flags.sh submission to determine
# the presence of an observation constraint within a few days at 
# each location. To be followed by submit_masking_calendar.sh.

# parameters
LSF_FILENAME=masking_job_window.sh  # script to run
...
# windowing settings to apply
export WINDOW_RANGE=10
export COUNT_THRESHOLD=5

export LOGDIR="/work/scratch/cmorice/lsf/threshold_window/${ANALYSIS_VERSION}/"
export INPUT_DIRECTORY="/work/scratch/cmorice/masking/${ANALYSIS_VERSION}/threshold_C${CLIMATOLOGY_THRESHOLD}_L${LARGESCALE_THRESHOLD}_T${CONSTRAINT_THRESHOLD}/"
export OUTPUT_DIRECTORY="/work/scratch/cmorice/masking/${ANALYSIS_VERSION}/threshold_C${CLIMATOLOGY_THRESHOLD}_L${LARGESCALE_THRESHOLD}_T${CONSTRAINT_THRESHOLD}_window_W${WINDOW_RANGE}_C${COUNT_THRESHOLD}/"
...
BATCHARRAY="masking[$BATCHSTART-$BATCHEND]"
