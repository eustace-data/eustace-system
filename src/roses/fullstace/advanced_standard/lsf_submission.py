...
def submit_eumopps_jobs(cataloguefilename, module_name, wait_options="", lsf_script = LSF_BATCH_SCRIPT, eumopps_batchsize = 366, output_directory=OUTDIR):
    ...
    for eumopps_batchnumber in range(batchcount):
            ...
            #continue
        #if not eumopps_batchnumber in [161, 162]:
        #    continue
...
def eustace_iteration(interation_number,
    ...
    if local_input_and_solve:
        if not local_hyperparameters:
            #if interation_number != 2:
            job_ids = submit_eumopps_jobs(cataloguefilename, 'local_input_and_solve_'+str(interation_number), wait_options=wait_options, lsf_script = LSF_BATCH_SCRIPT_WIDE, output_directory=OUTDIR)
        else:
            job_ids = submit_eumopps_jobs(cataloguefilename, 'nonstationary_local_input_and_solve_'+str(interation_number), wait_options=wait_options, lsf_script = LSF_BATCH_SCRIPT_WIDE, output_directory=OUTDIR)
        wait_options = done_conditon(job_ids)
